...
int main() {
    ...
    cout<<"The threadcount is: "<<threadcount<<endl;
    cout<<endl;
...
*/
    ...
    cout<< "---Advanced AVX parallelized k-means algorithm---"<<endl;
    cout << "Execution time: " << time_avx_align << " seconds" << endl;
    cout<<endl;

    auto start_avx_pthread = chrono::steady_clock::now();
    kMeans_avx_pthread();
    auto end_avx_pthread = chrono::steady_clock::now();

    double accuracy_avx_pthread = getAccuracy();
    double time_avx_pthread = chrono::duration_cast<chrono::microseconds>(end_avx_pthread - start_avx_pthread).count() / 1000000.0;

    //cout << "Accuracy: " << accuracy_avx_align << endl;
    cout<< "---Pthread and AVX based k-means algorithm---"<<endl;
    cout << "Execution time: " << time_avx_pthread << " seconds" << endl;
    return 0;
}
...
void* findlabel(void* rankid) {

    double minDist = numeric_limits<double>::max();
    int label = -1;
    //cout<<"findlabel()"<<endl;
    size_t id = (size_t)rankid;
    //cout<<id<<endl;
    for (int i = id*(N/threadcount); i < (id+1)*(N/threadcount); ++i) {
        __m256d kmeans_data_i = _mm256_load_pd(&kmeans_data[i][0]);
        for (int j = 0; j < K; ++j) {
            __m256d centroid_j = _mm256_load_pd(&centroids[j][0]);
            double dist_sum = calculate_euclidean_distance(kmeans_data_i, centroid_j);
...
void kMeans_avx_pthread() {
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        for (int i = 0; i < threadcount; ++i) {
            pthread_create(&tid[i], NULL, findlabel, (void*)i);
        }
        for (int i = 0; i < threadcount; ++i) {
